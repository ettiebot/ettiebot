...
import type { langCode, langCodeWithAuto } from "../../types/Translator";
...
export class GoogleTranslator extends AbstractGoogleTranslator {
			...
			return axios
				...
				.then((rsp) => {
					...
					const translatedText = rsp[0]
						.map((chunk) =>
							chunk instanceof Array && typeof chunk[0] === "string" ? chunk[0] : "",
						)
						.join("");
	...
	private parseXMLResponse = (text: string) => {
		try {
			...
			return nodes.length === 0 ? null : nodes.map((node) => node.toString()).join(" ");
		} catch (err) {
			return null;
		}
	};
...
export class GoogleTranslatorTokenFree extends AbstractGoogleTranslator {
	...
	translate = async (text: string, from: langCodeWithAuto, to: langCode) => {
