...
import { BaseTranslator } from '../../util/BaseTranslator.js';

import { getYandexSID } from './getYandexSID.js';
...
export class YandexTranslator extends BaseTranslator {
  ...
  public async translateBatch(text: string[], from: string, to: string) {
    ...
    let body = stringify.stringify(options);
    ...
    return fetch(this.wrapUrlToCorsProxy(urlWithSid + body), {
      ...
      return resp.text.map((text: string) => ({
        text: unescape(text),
        from: resp.lang.split('-')[0],
        to: resp.lang.split('-')[1],
      }));
    });
  }
}
